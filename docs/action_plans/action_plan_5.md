...
1. [ ] **Research platform limits and choose the background voice approach**
   - Exact files to create or modify: `docs/background_voice_constraints.md`, optionally `docs/action_plans/action_plan_5.md`
   - What must be implemented: Document what iOS and Android realistically allow for microphone capture, speech recognition, TTS playback, foreground services, lock-screen behavior, and app backgrounding. Decide the MVP strategy for each platform before touching native code.
...
2. [ ] **Add audio session configuration layer**
   ...
   - What must be implemented: Create a service that configures audio session behavior for listening and speaking, including microphone input, speaker/headphones routing, interruption handling hooks, and active/inactive session transitions.
...
3. [ ] **Configure iOS background audio and permissions**
...
4. [ ] **Configure Android foreground service and audio focus**
...
5. [ ] **Add app lifecycle handling to voice state**
   - Exact files to create or modify: `lib/features/voice/application/voice_controller.dart`, `lib/features/voice/domain/voice_state.dart`, `lib/features/voice/data/speech_to_text_service.dart`, `lib/features/voice/data/text_to_speech_service.dart`
...
6. [ ] **Handle audio interruptions and route changes**
...
7. [ ] **Add background continuation UX feedback**
...
8. [ ] **Add tests for lifecycle, interruption, and fallback states**
...
10. [ ] **Finalize graceful degradation and documentation**
